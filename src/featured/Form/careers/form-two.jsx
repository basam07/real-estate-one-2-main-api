...
const FormTwo = ({ formData = null }) => {
  ...
  const handleCreate = async (data) => {
    try {
      const response = await axios.post("/api/career-realivte", data);
  ...
  const handleUpdate = async (data) => {
    try {
      await axios.put(`/api/about-realivte/${formData?.id}`, data);
      ...
      >
        ...
        <div className="mb-4">
          <FormField
            ...
            defaultValue={formData?.realinvteTitle}
            required
          />
        </div>
        ...
        <div className="mb-4">
          <FormTextBox
            label="Description"
            type="text"
            name="realinvteDescription"
            placeholder="Enter Description"
            defaultValue={formData?.realinvteDescription}
            required
          />
        </div>
