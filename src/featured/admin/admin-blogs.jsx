...
import DeleteBlogsPageButton from "@/components/admin/blogs/delete-blogs-page-button";
      ...
      <div className="flex flex-wrap gap-5 w-11/12 mx-auto">
        {blogsData.map((insight, index) => (
          <div
            ...
            className="grid-item mx-auto"
            key={index}
          >
            <Link href={`/blogs/${insight.slug}`}>
              <Image
                src={ImageBasePath + insight.heroImage}
                width={300}
                height={300}
            ...
            <div style={{ padding: "15px" }} className="info bg-white">
              ...
              <div>{insight.content.slice(0, 100)}</div>
              <div>
                <div className="buttons mt-4 text-white">
                  <Link
                    className={"text-white py-2 px-4 rounded mr-2 bg-[#5AB2FF]"}
                    href={`/admin/blogs/${insight.slug}`}
                  >
                    Edit
                  </Link>
